...
"$repository_root/scripts/package-native.sh" \
  "$temporary/export" v0.4.1 18 "$temporary/release" >/dev/null

archive="$temporary/release/openapi_fdw-v0.4.1-pg18-linux-amd64.tar.gz"
...
test -f "$temporary/install/lib/openapi_fdw-0.4.1.so"
test ! -e "$temporary/install/lib/openapi_fdw.so"
test -f "$temporary/install/share/extension/openapi_fdw.control"
test -f "$temporary/install/share/extension/openapi_fdw--0.4.1.sql"
...
test -f "$temporary/install/share/extension/openapi_fdw--0.4.0--0.4.1.sql"
