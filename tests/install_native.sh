...
"$repository_root/scripts/package-native.sh" \
  "$temporary/export" v0.3.2 18 "$temporary/release" >/dev/null

archive="$temporary/release/openapi_fdw-v0.3.2-pg18-linux-amd64.tar.gz"
...
test -f "$temporary/install/lib/openapi_fdw-0.3.2.so"
test ! -e "$temporary/install/lib/openapi_fdw.so"
test -f "$temporary/install/share/extension/openapi_fdw.control"
test -f "$temporary/install/share/extension/openapi_fdw--0.3.2.sql"
