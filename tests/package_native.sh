...
printf 'library\n' >"$temporary/export/usr/lib/postgresql/18/lib/openapi_fdw-0.4.1.so"
printf 'control\n' >"$temporary/export/usr/share/postgresql/18/extension/openapi_fdw.control"
printf 'sql\n' >"$temporary/export/usr/share/postgresql/18/extension/openapi_fdw--0.4.1.sql"
printf 'old-upgrade\n' >"$temporary/export/usr/share/postgresql/18/extension/openapi_fdw--0.3.1--0.3.2.sql"
printf 'prior-upgrade\n' >"$temporary/export/usr/share/postgresql/18/extension/openapi_fdw--0.3.2--0.4.0.sql"
printf 'upgrade\n' >"$temporary/export/usr/share/postgresql/18/extension/openapi_fdw--0.4.0--0.4.1.sql"

"$repository_root/scripts/package-native.sh" \
  "$temporary/export" v0.4.1 18 "$temporary/output" >/dev/null

archive="$temporary/output/openapi_fdw-v0.4.1-pg18-linux-amd64.tar.gz"
(cd "$temporary/output" && sha256sum --check "${archive##*/}.sha256" >/dev/null)
tar -C "$temporary/unpacked" -xzf "$archive"

test -f "$temporary/unpacked/openapi_fdw-0.4.1.so"
test -f "$temporary/unpacked/openapi_fdw.control"
test -f "$temporary/unpacked/openapi_fdw--0.4.1.sql"
test -f "$temporary/unpacked/openapi_fdw--0.3.1--0.3.2.sql"
test -f "$temporary/unpacked/openapi_fdw--0.3.2--0.4.0.sql"
test -f "$temporary/unpacked/openapi_fdw--0.4.0--0.4.1.sql"
